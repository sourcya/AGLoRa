...
void AGLORA::checkMemoryToBLE() {
  String response = bleProtocolPrefix + bleProtocolTypeMemory + bleProtocolVersion;
...
LORA::LORA(uint8_t pinRx, uint8_t pinTx, long speed, uint8_t aux, uint8_t m0, uint8_t m1, INDICATION *indication)
  : loraPort(pinRx, pinTx),
...
bool EEPROMAglora::checkUnique(DATA *newPoint) {
  ...
  for (unsigned int i = 0; i < EEPROMStorageSize; i++) {
    ...
    if ((newPoint->name == eepromPoint->name) && (newPoint->year == eepromPoint->year) && (newPoint->month == eepromPoint->month) && (newPoint->day == eepromPoint->day) && (newPoint->hour == eepromPoint->hour) && (newPoint->minute == eepromPoint->minute) && (newPoint->second == eepromPoint->second)) {
#if DEBUG_MODE && DEBUG_MEMORY
...
bool SRAM::checkUnique(DATA *newPoint) {
  ...
  for (unsigned int i = 0; i < maxIndex; ++i) {
    ...
    if ((strcmp(newPoint->name, storage[i].data.name) == 0) && (newPoint->year == storage[i].data.year) && (newPoint->month == storage[i].data.month) && (newPoint->day == storage[i].data.day) && (newPoint->hour == storage[i].data.hour) && (newPoint->minute == storage[i].data.minute) && (newPoint->second == storage[i].data.second)) {
#if DEBUG_MODE && DEBUG_MEMORY
      Serial.println();
  ...
  for (unsigned int i = 0; i < SRAM_STORAGE_SIZE; ++i) {
    if (i <= maxIndex) {
      ...
      if (crc < 10)
        Serial.print(F("0"));
      Serial.print(crc);
      if ((i == storageIndex - 1) || ((i == 0) && (storageOverwrite))) {
